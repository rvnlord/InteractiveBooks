...
namespace MVCDemo.Controllers
{
    public class BookController : BaseController
    {
        ...
        public BookContent GetXmlBookContent(Book book)
        {
            ...
            // nadpisz niepoprawne wartości w plikach XML wartościami z bazy danych
            if (!string.Equals(root?.Element("Title")?.Value, book.Title, StringComparison.OrdinalIgnoreCase))
            {
                var xTitle = root?.Element("Title");
                if (xTitle != null)
                    xTitle.Value = book.Title;
            }

            if (!string.Equals(root?.Element("Author")?.Value, book.Author.UserName, StringComparison.OrdinalIgnoreCase))
            {
                var xAuthor = root?.Element("Author");
                if (xAuthor != null)
                    xAuthor.Value = book.Author.UserName;
            }
